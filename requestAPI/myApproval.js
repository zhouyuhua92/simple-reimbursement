...
const searchWaitApprovePage = (data, success, fail) => {
    request.postFetch(`/api/edo-desk/task/searchWaitApprovePage?current=${data.current}&size=${data.size}`, data, success, fail);
}
//查询已办列表  开发人：周鑫
const searchApprovedPage = (data, success, fail) => {
  request.postFetch(`/api/edo-desk/task/searchApprovedPage?current=${data.current}&size=${data.size}`, data, success, fail);
}
//查询我的发起  开发人：周鑫
const searchStartPage = (data, success, fail) => {
  request.postFetch(`/api/edo-desk/task/searchStartPage?current=${data.current}&size=${data.size}`, data, success, fail);
}
//审批 开发人：周鑫
